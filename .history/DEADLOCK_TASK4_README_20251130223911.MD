...
```
| Model Name | Description | Expected Result | Actual Result | Reachable States (RS) | Execution Time (s) | Status |
|:----------|:----------|:----------|:----------|:----------|:----------|:----------|
| case_no_deadlock.pnml | Simple loop structure, infinite run. | NO DEADLOCK | NO DEADLOCK | X | 0.00xxx | PASS |
| case_choice_deadlock.pnml | Non-deterministic choice leading to a sink state. | DEADLOCK | DEADLOCK | X | 0.00xxx | PASS |
| case_resource_deadlock.pnml | Resource contention (Circular Wait). | DEADLOCK | DEADLOCK | X | 0.00xxx | PASS |
| deadlock_task4.pnml | Sequential flow ending in a sink. | DEADLOCK | DEADLOCK | X | 0.00xxx | PASS |
```

(Note: Replace X and 0.00xxx with the actual numbers from your terminal output)
3. **Analysis**
...
## **DETAIL UPDATE FOR TASK 4 - THEHOANG**
